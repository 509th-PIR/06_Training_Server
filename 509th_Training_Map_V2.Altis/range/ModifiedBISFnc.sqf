...
if (typename _target == typename objnull) then {
	...
	if ((_target getvariable ["bis_fnc_VRHitParts_handler",-1]) < 0) then {
		...
		{_hiddenselections set [_foreachindex,tolower _x];} foreach _hiddenselections;
		{
			...
			_target setobjectmaterial [_foreachindex,MAT_GREEN];
				...
				_target setvariable [
					...
					_target spawn {
						...
						for "_i" from 0 to 10 do {
							...
							sleep 0.05;
							{_target setobjectmaterial [_foreachindex,MAT_GREEN];} foreach _materials;
							sleep 0.05;
						};
...
pBIS_fnc_garage = {
if (is3DEN && {_fnc_scriptName == "pBIS_fnc_garage"}) exitwith {_this call pBIS_fnc_garage3DEN;};
...
switch _mode do {
	...
	case "Open": {
		...
		with missionnamespace do {
			pBIS_fnc_garage_center = [_this,1,missionnamespace getvariable ["pBIS_fnc_garage_center",player],[objnull]] call bis_fnc_param;
		};
		with uinamespace do {
	...
	case "Init": {
		...
		pBIS_fnc_garage_turretPaths = [];
		if (isnil "pBIS_fnc_garage_centerType") then {pBIS_fnc_garage_centerType = "";};
		setstatvalue ["MarkVirtualVehicleInspection",1];

		with missionnamespace do {
			BIS_fnc_arsenal_group = creategroup side group player;
			BIS_fnc_arsenal_center = missionnamespace getvariable ["pBIS_fnc_garage_center",player];
		};

		//--- Show specific class
		_classDefault = uinamespace getvariable ["pBIS_fnc_garage_defaultClass",""];
		if (isclass (configfile >> "cfgvehicles" >> _classDefault)) then {
			pBIS_fnc_garage_centerType = gettext (configfile >> "cfgvehicles" >> _classDefault >> "model");
		};
		uinamespace setvariable ["pBIS_fnc_garage_defaultClass",nil];

		//--- Load stats
		if (isnil {uinamespace getvariable "pBIS_fnc_garage_stats"}) then {
			_defaultCrew = gettext (configfile >> "cfgvehicles" >> "all" >> "crew");
			uinamespace setvariable [
				"pBIS_fnc_garage_stats",
				[
		...
		INITTYPES
		["InitGUI",[_display,"pBIS_fnc_garage"]] call bis_fnc_arsenal;
		["Preload"] call pBIS_fnc_garage;
		["ListAdd",[_display]] call pBIS_fnc_garage;
		//["ListSelectCurrent",[_display]] call pBIS_fnc_garage;
		if (pBIS_fnc_garage_centerType == "") then {["buttonRandom",[_display]] call pBIS_fnc_garage;};
	...
	case "Exit": {
		with missionnamespace do {
			pBIS_fnc_garage_center = BIS_fnc_arsenal_center;
/*
			{
...
*/
		...
		pBIS_fnc_garage_turretPaths = nil;
	...
	case "Preload": {
		if (is3DEN) then {
			...
			["pBIS_fnc_garage_preload"] call bis_fnc_startloadingscreen;
			private ["_data"];
			_data = [];
			...
			missionnamespace setvariable ["pBIS_fnc_garage_data",_data];
			["pBIS_fnc_garage_preload"] call bis_fnc_endloadingscreen;
			pBIS_fnc_garage_centerType = typeof _center;
			true
		} else {
	...
	case "ListAdd": {
		_display = _this select 0;
		_data = missionnamespace getvariable "pBIS_fnc_garage_data";
		_center = (missionnamespace getvariable ["BIS_fnc_arsenal_center",player]);
		_checkboxTextures = [
		...
		if (is3DEN) then {
			...
			["ShowItemStats",[_cfg]] call pBIS_fnc_garage;
		} else {
			{
				_items = _x;
				_ctrlList = _display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _foreachindex);
				...
				for "_i" from 0 to (lbsize _ctrlList - 1) do {
					if ((_ctrlList lbdata _i) == pBIS_fnc_garage_centerType) then {
...
*/
		{
			...
			if (_active) then {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _idc),_idc]] call pBIS_fnc_garage;
			} else {
		...
		['TabSelectRight',[_display,IDC_RSCDISPLAYGARAGE_TAB_SUBCREW]] call pBIS_fnc_garage;
	};
		...
		switch _idc do {
			...
			case IDC_RSCDISPLAYGARAGE_TAB_STATIC: {
				...
				if (_class != _centerType || !alive _center) then {
					...
					pBIS_fnc_garage_centerType = _item;
				...
				if (getnumber (_cfg >> "isUAV") < 1) then {
					...
					_proxyIndexes = [];
					{
						...
						pBIS_fnc_garage_turretPaths pushback _x;
			...
			case IDC_RSCDISPLAYGARAGE_TAB_SUBCREW: {
				...
				switch _role do {
					...
					case "Turret": {
						_roleIndexArray = pBIS_fnc_garage_turretPaths select _roleIndex;
...
*/
		...
		["SetCrewStatus",[_display]] call pBIS_fnc_garage;
		if (isclass _cfg) then {
			["ShowItemInfo",[_cfg]] call bis_fnc_arsenal;
			["ShowItemStats",[_cfgStats]] call pBIS_fnc_garage;
		};
	};
	...
	case "SetCrewStatus": {
		...
		for "_i" from 0 to (lbsize _ctrlListCrew - 1) do {
			...
			switch (tolower _role) do {
				...
				case "commander";
				case "turret": {
					_turretPath = pBIS_fnc_garage_turretPaths select _roleIndex; 
	...
	case "ShowItemStats": {
		...
		if (isclass _itemCfg) then {
			...
			_statsExtremes = uinamespace getvariable "pBIS_fnc_garage_stats";
			...
			if (_categoryIndex >= 0) then {
				...
				if (_nameID >= 0) then {
					...
					[_center,_name,_categoryIndex,_display] spawn { //--- Must be spawned, animations cannot be applied immediately
						...
						if (is3DEN) then {
							['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _categoryIndex),_categoryIndex]] call pBIS_fnc_garage;
						} else {
