...
NGT_fnc_addBulletTrackingActions = {
	if (!isDedicated) then {
		...
		_actions		= [
			...
			["Add BT To Vehicle", {[cursorTarget, nil, 1] call NGT_fnc_addBulletTrackingHandler}, "(!isNil 'NGT_bt_handle_draw3DID') && (cursorTarget isKindOf 'AllVehicles') && ((cursorTarget getVariable ['NGT_bt_int_trackingHandle', -1]) < 0)"],
			...
			["Add BT To Self", {[_this select 1,nil,1] call NGT_fnc_addBulletTrackingHandler}, "(!isNil 'NGT_bt_handle_draw3DID') && ((_this getVariable ['NGT_bt_int_trackingHandle', -1]) < 0)"],
...
[player, 452] call NGT_fnc_addBulletTrackingActions;
player addEventHandler ["Respawn",{[player, 452] call NGT_fnc_addBulletTrackingActions}];
/*
{
[_x, 4, 2] call NGT_fnc_addBulletTrackingHandler;
} foreach Allunits;*/